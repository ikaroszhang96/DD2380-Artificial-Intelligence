...
public:
     ...
     */
    Player();
    std::vector<std::vector<HMM>> hmms;
    /**
     * Shoot!
     *
    ...
    std::vector<EMovement> getObservations(const Bird &b);
    std::vector<EMovement> getObservations70(const Bird &b);
    ESpecies predict(const Bird &b);
};
